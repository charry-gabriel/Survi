...
/**
 * Charge tous les fichiers {@code growth_items/<id>.yml}, les convertit en
 * {@link GrowthConfig} et les enregistre dans {@link GrowthItemRegistry}.
 *
 * <p>Appelé une seule fois depuis {@link GrowthItems#init()}.
 */
public final class GrowthItemLoader {
    ...
    private static ItemEffect convertEffect(EffectConfig e) {
        return switch (e.type) {
            case "name" ->
                    new NameItemEffect(Component.text(e.value));

            case "message" ->
                    new MessageItemEffect(Component.text(e.value));

            case "haste" ->
                    new HasteItemEffect(e.seconds);

            case "add_enchantment" ->
                    new AddEnchantmentItemEffect(parseEnchantment(e.enchantment), e.amount);

            case "set_attribute" ->
                    new SetAttributeItemEffect(
                            parseAttribute(e.attribute),
                            e.attributeValue,
                            parseOperation(e.operation),
                            parseSlotGroup(e.slot));
    ...
    /**
     * Résout un enchantement depuis sa clé Minecraft en minuscule (ex. {@code fortune}).
     * Utilise {@link Enchantment#getByKey(NamespacedKey)} (deprecated mais stable en Paper 26).
     */
    ...
    private static Enchantment parseEnchantment(String name) {
        ...
        if (ench == null)
            throw new IllegalArgumentException("Enchantement inconnu : '" + name + "' (vérifier la clé minecraft)");
        return ench;
    }

    /**
     * Résout un {@link Attribute} depuis son nom en snake_case (ex. {@code mining_efficiency}).
     * La liste est exhaustive pour les attributs utilisés dans les growth items.
     * Ajouter ici si un nouveau growth item nécessite un attribut supplémentaire.
     */
